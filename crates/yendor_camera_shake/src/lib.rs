...
impl CameraShake {
    /// Create a new [`CameraShake`] component with the provided maximum offset angle (in
    /// degrees) and position as well as the trauma decay rate in seconds.
    ...
    /// Create a new [`CameraShake`] component with the provided maximum offset angle (in
    /// degrees) and position and its initial trauma set to some value (clamped between 0
    /// and 1).
