...
fn impl_type_ulid(input: &syn::DeriveInput) -> TokenStream2 {
    ...
    for attr in &input.attrs {
        ...
        if name_value.path.get_ident().map(|i| i != "ulid").unwrap_or(true) {
            continue;
        }
    ...
    let id = ulid.0;
    quote! {
        impl ::yendor_type_ulid::TypeUlid for #item_ident {
            const ULID: ::yendor_type_ulid::Ulid = ::yendor_type_ulid::Ulid(#id);
        }
    }
}
