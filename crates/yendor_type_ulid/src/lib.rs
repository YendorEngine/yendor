...
pub use ulid::Ulid;
pub use yendor_type_ulid_macros::TypeUlid;

/// Associates a [`Ulid`] ID to a Rust type.
///
/// > **⚠️ Warning:** there is nothing enforcing that the [`Ulid`]s returned by different types
/// > will
/// > be different.
pub trait TypeUlid {
    /// The type's [`Ulid`].
...
/// Allows reading a type's [`Ulid`] from the context of a trait object when the concrete Rust
...
mod test {
    ...
    #[test]
    fn type_ulid_dyanmic_is_object_safe() { let _: Box<dyn TypeUlidDynamic> = Box::new(()); }
}
