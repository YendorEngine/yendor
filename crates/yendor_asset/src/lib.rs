...
/// A resource that may be used to access [`AssetProvider`]s for all the different registered
/// asset types.
///
/// > ⚠️ **Warning:** This API is work-in-progress and has not been used in an actual project
/// > yet.
/// > In the official Bevy integration we are currently "cheating" by just borrowing asset
/// > directly
/// > from the Bevy world.
/// >
/// > Cheating like this means that we can't acess assets through the C API that we want to
/// > provide
...
impl Default for AssetProvidersResource {
    fn default() -> Self {
        Self(Arc::new(atomic_refcell::AtomicRefCell::new(
...
impl AssetProviders {
    ...
    pub fn get<T: TypeUlid>(&self) -> AssetProviderRef<T> { self.try_get::<T>().unwrap() }
    ...
    /// Get the asset provider for the given type
    pub fn get_mut<T: TypeUlid>(&mut self) -> AssetProviderMut<T> { self.try_get_mut::<T>().unwrap() }
    ...
    pub fn remove<T: TypeUlid>(&mut self) -> Box<dyn UntypedAssetProvider> { self.try_remove::<T>().unwrap() }
...
pub trait UntypedAssetProvider: Sync + Send {
    ...
    /// Returns a mutable-only pointer to the asset for the given handle, or a null pointer if
    /// it doesn't exist.
...
///
/// Asset providers are reponsible for returning references to assets out of their backing
/// asset store, when giving handles to the asset to laod
...
impl<T: TypeUlid> UntypedAssetProvider for dyn AssetProvider<T> {
    fn get(&self, handle: UntypedHandle) -> *const u8 {
        ...
        asset.map(|x| x as *const T as *const u8).unwrap_or(std::ptr::null())
    }

    fn get_mut(&mut self, handle: UntypedHandle) -> *mut u8 {
        let asset = <Self as AssetProvider<T>>::get_mut(self, handle.typed());
        asset.map(|x| x as *mut T as *mut u8).unwrap_or(std::ptr::null_mut())
    }
}
...
impl AssetPath {
    ...
    /// Take this path, treat it as a path relative to `base_path`, normalize it, and update
...
///
/// The type of the handle is used to help reduce runtime errors arising from mis-matching
/// handle types, but internally, the handle's only stored data is it's [`AssetPath`].
///
...
impl<T: TypeUlid> Handle<T> {
    ...
    pub fn untyped(self) -> UntypedHandle { UntypedHandle { path: self.path } }
}
...
impl<'de, T: TypeUlid> serde::Deserialize<'de> for Handle<T> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where D: serde::Deserializer<'de> {
        deserializer.deserialize_str(UntypedHandleVisitor).map(UntypedHandle::typed)
    }
}
