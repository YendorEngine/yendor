...
fn impl_has_load_progress(input: &syn::DeriveInput) -> TokenStream2 {
    ...
    if skip_all_fields {
        impl_function_body = quote! {
            yendor_has_load_progress::LoadProgress::default()
        };
    ...
    // If we should process struct fields
        ...
        'field: for field in &in_struct.fields {
            ...
            progresses.push(quote_spanned! { field_ident.span() =>
                yendor_has_load_progress::HasLoadProgress::load_progress(
                    &self.#field_ident,
                    loading_resources
                )
            })
        ...
        impl_function_body = quote! {
            #impl_function_body
            yendor_has_load_progress::LoadProgress::merged([ #( #progresses),* ])
        };
    }

    // Fill out rest of impl block
    quote! {
        impl yendor_has_load_progress::HasLoadProgress for #item_ident {
            fn load_progress(
                &self,
