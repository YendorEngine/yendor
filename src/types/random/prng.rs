...
pub struct Prng<Rng: RngCore + SeedableRng> {
    seed: u64,
    rng: Box<Rng>,
}

#[allow(dead_code)]
impl<Rng: RngCore + SeedableRng> Prng<Rng> {
    pub fn new(seed: u64) -> Self {
        Self {
            seed,
            rng: Box::new(Rng::seed_from_u64(seed)),
        }
    }

    pub fn from_entropy() -> Self { Self::new(Rng::from_entropy().next_u64()) }

    pub fn as_rng(&mut self) -> &mut Rng { &mut self.rng }
    ...
    pub fn entropy() -> u32 { Rng::from_entropy().gen::<u32>() }

    pub fn entropy_u64() -> u64 { Rng::from_entropy().gen::<u64>() }

    pub fn entropy_f32() -> f32 {
        (Rng::from_entropy().gen::<u32>() as f64 / (u32::MAX as u64 + 1) as f64) as f32
    }

    pub fn entropy_f64() -> f64 { Rng::from_entropy().gen::<u64>() as f64 / (u64::MAX as u128 + 1) as f64 }
...
impl<R: RngCore + SeedableRng> Iterator for Prng<R> {
    type Item = u64;

    fn next(&mut self) -> Option<Self::Item> { Some(self.next_u64()) }
}
