...
macro_rules! impl_grid_point_tuple {
    ($type:ty) => {
        impl Point for $type {
            fn x_int32(&self) -> i32 { self.0 as i32 }

            fn y_int32(&self) -> i32 { self.1 as i32 }

            fn x_uint32(&self) -> u32 { self.0 as u32 }

            fn y_uint32(&self) -> u32 { self.1 as u32 }

            fn x_float32(&self) -> f32 { self.0 as f32 }

            fn y_float32(&self) -> f32 { self.1 as f32 }
        }
    };
}
...
impl Point for (f32, f32) {
    fn x_int32(&self) -> i32 { self.0.floor() as i32 }

    fn y_int32(&self) -> i32 { self.1.floor() as i32 }

    fn x_uint32(&self) -> u32 { self.0.floor() as u32 }

    fn y_uint32(&self) -> u32 { self.1.floor() as u32 }

    fn x_float32(&self) -> f32 { self.0 }

    fn y_float32(&self) -> f32 { self.1 }
}
