...
impl Rectangle {
    ...
    fn find_min_max(min: ChunkPosition, max: ChunkPosition) -> (ChunkPosition, ChunkPosition) {
        ...
        (
            ChunkPosition::from_absolute(min_x, min_y, min_z),
            ChunkPosition::from_absolute(max_x, max_y, max_z),
        )
    }
}
...
impl Rectangle {
    #[inline]
    pub fn width(&self) -> u32 { self.max.distance_x(self.min) }

    #[inline]
    pub fn height(&self) -> u32 { self.max.distance_y(self.min) }
