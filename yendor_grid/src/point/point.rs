...
pub trait Point: Clone + Copy {
    ...
    #[inline]
    fn lerp(self, other: impl Point, t: f32) -> IVec2 { self.as_vec2().lerp(other.as_vec2(), t).as_ivec2() }
    ...
    /// NW)
    #[inline]
    fn neighbors_all(&self) -> AdjIterator { AdjIterator::new(*self, DirectionIter::all_2d()) }

    /// Returns an iterator over the 4 points cardinal - adjacent to this one. (N, E, S, W)
    #[inline]
    fn neighbors_cardinal(&self) -> AdjIterator { AdjIterator::new(*self, DirectionIter::cardinal()) }

    /// Returns an iterator over the 4 points ordinal - adjacent to this one. (NE, SE, SW,
    /// NW)
    #[inline]
    fn neighbors_ordinal(&self) -> AdjIterator { AdjIterator::new(*self, DirectionIter::ordinal()) }
}
